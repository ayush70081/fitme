...
@router.post("/save", response_model=MealPlanResponse)
async def save_meal_plan(
    meal_plan: MealPlan = Body(...),
    current_user: User = Depends(get_current_active_user)
):
    """Save a daily meal plan for the user"""
    try:
        saved_plans_collection = await get_collection("saved_meal_plans")
        saved_plan = SavedMealPlan(
            user_id=current_user.id,
            plan=meal_plan,
            saved_at=datetime.utcnow()
        )
        result = await saved_plans_collection.insert_one(saved_plan.dict(exclude={"id"}))
        saved_plan.id = str(result.inserted_id)
        return MealPlanResponse(success=True, message="Plan saved", data=meal_plan)
    except Exception as e:
        logger.error(f"Error saving meal plan: {e}")
        raise HTTPException(status_code=500, detail="Failed to save meal plan")

@router.get("/saved", response_model=MealPlanResponse)
async def get_saved_meal_plan(
    current_user: User = Depends(get_current_active_user)
):
    """Get the latest saved meal plan for the user"""
    try:
        saved_plans_collection = await get_collection("saved_meal_plans")
        doc = await saved_plans_collection.find_one(
            {"user_id": current_user.id},
            sort=[("saved_at", -1)]
        )
        if not doc:
            return MealPlanResponse(success=False, message="No saved plan found", data=None)
        plan = doc["plan"]
        return MealPlanResponse(success=True, message="Loaded saved plan", data=MealPlan.parse_obj(plan))
    except Exception as e:
        logger.error(f"Error loading saved meal plan: {e}")
        raise HTTPException(status_code=500, detail="Failed to load saved meal plan")